...
const Completed = () => {
  ...
  useEffect(() => {
    GetTaskList("Completed", 1, perPageKey, searchKey);
  }, []);
  ...
  const handlePageClick = (event) => {
    GetTaskList("Completed", event.selected + 1, perPageKey, searchKey);
  };

  const PageKeyOnChange = (e) => {
    setPerPageKey(parseInt(e.target.value));
    GetTaskList("Completed", 1, e.target.value, searchKey);
  };
  ...
  const searchOnChange = (e) => {
    ...
    if (e.target.value.length === 0) {
      setSearchKey("0");
      GetTaskList("Completed", 1, perPageKey, "0");
    }
  };
